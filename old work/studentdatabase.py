...
if menu == "View Database|Chart":
    ...
    st.table(csvlink)
    subjects = ["English","Science","Art","PE","Health"]
    subjectstable = csvlink[subjects].mean().reset_index()
    st.table(subjectstable)
    chart = st.radio("Choose Chart Type:", ["Bar Chart", "Pie Chart"])
    if chart == "Bar Chart":
        barchart = px.bar(subjectstable, x="index", y=0, labels = {"index":"Subject",0:"Average Score"})
        st.plotly_chart(barchart)
    else:
        piechart = px.pie(subjectstable, names="index", values=0, labels = {"index":"Subject",0:"Average Score"})
